/*
寮€鍗℃椿鍔�
娲诲姩鍏ュ彛锛歨ttps://u.jd.com/w6srT2x
涓€娆℃€ф椿鍔紝杩愯瀹岃剼鏈幏寰�53浜眴锛岃繘鍏ュ叆鍙ｈ繕鍙互寮€鍗￠30閮�
*/
const $ = new Env('寮€鍗℃椿鍔�');
      ...
      if (cookiesArr[i]) {
        ...
        if (!$.isLogin) {
          ...
          if ($.isNode()) {
            ...
            $.setdata('', `CookieJD${i ? i + 1 : ""}`);//cookie澶辨晥锛屾晠娓呯┖cookie銆�$.setdata('', `CookieJD${i ? i + 1 : "" }`);//cookie澶辨晥锛屾晠娓呯┖cookie銆�
          }
          continue
        }
        await jdBeanHome();
            ...
            if(data['success']){
              for (let i = 0; i < data.data.myTasks.length; ++i){
                ...
                if(task.groupType===1) {
                  ...
                  $.log(`鍏虫敞搴楅摵浠诲姟锛屽畬鎴愭鏁�: ${task.finishCount}/${task.itemCount}`)
                } else if(task.groupType===2){
                  groupType = 'ADD_CART'
                  $.log(`鍔犺喘浠诲姟锛屽畬鎴愭鏁�: ${task.finishCount}/${task.itemCount}`)
                } else if (task.groupType===3){
                  groupType = 'FOLLOW_CHANNEL'
                  $.log(`閫涗細鍦轰换鍔★紝瀹屾垚娆℃暟: ${task.finishCount}/${task.itemCount}`)
                  if (task.finishCount< task.itemCount)
                    await $.wait(task.viewTime*1000)
                } else if (task.groupType===4){
                  groupType = 'VIEW_GOODS'
                  $.log(`娴忚鍟嗗搧浠诲姟锛屽畬鎴愭鏁�: ${task.finishCount}/${task.itemCount}`)
                }
              ...
              if (data.data.rewardStatus!==3) {
                $.log(`鍘婚鍙栭澶�${data.data.rewardFinish}浜眴`)
                await doTask("FINISH_TASK",0)
                const res = await doTask("FINISH_TASK",0)
                if(res['success']){
                  $.log(`棰嗗彇棰濆${data.data.rewardFinish}浜眴瀹屾垚鎴愬姛锛岃幏寰�${data.data.rewardFinish}浜眴`)
                }
              }
            } else{
              console.log('鑾峰彇澶辫触锛岃妫€鏌ユ槸鍚﹂粦鍙�')
            }
          }
        }
      } catch (e) {
